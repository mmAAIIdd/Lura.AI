 ...
 *
 * Владельца в сигнатурах нет — и это главное правило контракта. Экземпляр
 * хранилища привязан к одному владельцу в момент создания, через
 * studioStore(ownerId), и никакой его метод не умеет посмотреть в чужое
 * пространство: ни одним аргументом туда не дотянуться. Поэтому изоляция здесь
 * не проверка, о которой можно забыть, а форма самого объекта.
 *
 * Отсюда и соглашение для всего, что работает с хранилищем не само: привязанный
 * store передаётся ПЕРВЫМ параметром — indexDocument(store, ...),
 * saveReportToProject(store, ...), businessDocument(store). Один и тот же
 * порядок везде значит, что забытый аргумент — это ошибка компиляции в первой
 * же позиции, а не тихо подставленное значение по умолчанию.
 */
