...
**Разделение по владельцам.** Рабочее пространство у каждого владельца своё. На диске это `.lura-studio/<owner>/{documents,index,threads,artifacts,project}`, в Postgres — колонка `owner_id` в каждой таблице, которая входит в тот же `where`, что и идентификатор строки. Владелец — `sub` из токена Supabase, а при открытом входе (локальная разработка, `STUDIO_PUBLIC=true`) — `local`. Данные, оставшиеся от общей раскладки, приложение не переносит само: остановите его и перенесите папки вручную, иначе запись, идущая прямо сейчас, потеряется молча.

```powershell
mkdir .lura-studio\local
Move-Item .lura-studio\documents,.lura-studio\index,.lura-studio\threads,.lura-studio\artifacts,.lura-studio\project .lura-studio\local\
```
